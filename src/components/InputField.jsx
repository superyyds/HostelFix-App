...
const InputField = ({
  ...
  const handleFocus = (e) => {
    // If the field contains "N/A" (or null/undefined), clear it so user can type
    if (!readOnly && (value === 'N/A' || value === null || value === undefined)) {
      // Call parent's onChange with a compatible event shape
  ...
  return (
    <div className={`relative mb-5 ${className}`}>
      {Icon && <Icon className="absolute top-1/2 left-4 transform -translate-y-1/2 w-5 h-5 text-gray-400" />}
      <input
        name={name}
