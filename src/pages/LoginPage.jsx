...
const LoginPage = ({ onForgotPassword, onLoginFailure, onValidationFailure, onLoginSuccess, onMandatoryPasswordChange }) => { 
    ...
    }, [roleType, isMessageVisible]);

    const showMessage = (title, text, type, autoClose = true) => {
...
// } from '../api/firebase';
// import { wardenSessionCache } from '../api/cache';
// import { collection, query, where, getDocs } from "firebase/firestore"; // 🔥 ADD MISSING IMPORTS
// import { db } from '../api/firebase'; // 🔥 ADD THIS IMPORT
...
//             if (querySnapshot.empty) {
//                 console.log("🔴 DEBUG: No user data found for email:", email);
//                 return { isValid: false, error: "Account not found. Please check your email." };
//             }
...
//             if (!userData.role) {
//                 console.log("🔴 DEBUG: No role found in user data");
//                 return { isValid: false, error: "Account data incomplete. Please contact admin." };
//             }
            
//             const actualRole = userData.role;
//             console.log("🔍 DEBUG: Pre-auth actual role:", actualRole, "Selected role:", selectedRole);
...
//         } catch (error) {
//             console.error("🔴 DEBUG: Pre-auth validation error:", error);
//             // If pre-auth validation fails, we'll fall back to post-auth validation
//             return { isValid: true }; // Allow login attempt to proceed
//         }
//     };
...
//         if (validationError) {
//             console.log("🔴 DEBUG: Email validation failed:", validationError);
//             setIsLoading(false);
//             showMessage("Validation Error", validationError, "error");
//             return;
//         }
...
//         if (!preAuthValidation.isValid) {
//             console.log("🔴 DEBUG: Pre-auth role validation failed:", preAuthValidation.error);
//             setIsLoading(false);
//             showMessage("Role Mismatch", preAuthValidation.error, "error");
//             return; // STOP HERE - don't even attempt Firebase auth
//         }
...
//             if (!firestoreData || !firestoreData.role) {
//                 console.log("🔴 DEBUG: No role data found in Firestore");
//                 await signOut(auth);
//                 showMessage("Login Failed", "Your account data is incomplete. Please contact admin.", "error"); 
//                 setIsLoading(false);
//                 return;
...
//             // Final role mismatch check (should rarely trigger due to pre-auth check)
//             if (roleType !== actualUserRole) {
//                 console.log("🔴 DEBUG: Post-auth role mismatch detected");
...
//             // 🔥 CRITICAL ADDITION: Check for mandatory password change
//             if (firestoreData.mustChangePassword === true) {
//                 console.log("🟡 DEBUG: Mandatory password change required for user");
                
//                 await signOut(auth); // Sign out for security
                
//                 // Redirect to mandatory password change
//                 onMandatoryPasswordChange({
//                     userId: user.uid,
//                     userDocId: firestoreData.docId,
//                     userRole: actualUserRole,
//                     isVoluntary: false
//                 });
...
//         } catch (error) {
//             // --- Error Handling (Standard Firebase Auth Errors) ---
...
//                 validationError = "Invalid Student Email. Must be in the format 'xxx@student.usm.my'.";
//             }
//         } else if (roleType === 'warden' || roleType === 'staff') { // ⬅️ ADDED || roleType === 'staff'
//             const roleName = roleType.charAt(0).toUpperCase() + roleType.slice(1);
//             if (studentRegex.test(normalizedEmail)) {
//                 validationError = `Invalid ${roleName} Email. Please use a '@usm.my' address, not a student email.`;
//             } else if (!normalizedEmail.endsWith('@usm.my')) {
//                 validationError = `Invalid ${roleName} Email. Must be in the format 'xxx@usm.my'.`;
//             }
//         }
...
//         if (roleType === 'warden' || roleType === 'staff') { // ⬅️ ADDED || roleType === 'staff'
//             wardenSessionCache.email = normalizedEmail;
//             wardenSessionCache.password = password;
//         }
